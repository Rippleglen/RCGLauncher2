import { BrowserWindow, ipcMain, app, dialog, shell, screen } from "electron";
...
const API_BASE$1 = "https://minecraft.eggonomicsgame.com";
...
async function syncModpackFiles(modpack, instancePath) {
  ...
  const checkRes = await fetch(`${API_BASE$1}/sync/${encodeURIComponent(modpack.name)}`, {
    method: "POST",
  ...
  try {
    ...
    await downloadZip(
      `${API_BASE$1}/sync/${encodeURIComponent(modpack.name)}/download/${downloadToken}`,
      tempZip
    );
...
async function ensureModloader(modpack, instancePath, appDataPath2, onStatus) {
  ...
  if (loaderVersion.startsWith("neoforge-") || loaderVersion.startsWith("forge-")) {
    return ensureForge(modpack, instancePath, appDataPath2, onStatus);
  }
...
async function ensureForge(modpack, instancePath, appDataPath2, onStatus) {
  const { loaderVersion, mcVersion } = modpack;
  const isNeoForge = loaderVersion.startsWith("neoforge-");
  const ver = loaderVersion.replace(/^(neoforge-|forge-)/, "");
  const customId = isNeoForge && ver.startsWith("47.") ? `1.20.1-forge-${ver}` : isNeoForge ? `neoforge-${ver}` : `${mcVersion}-forge-${ver}`;
  const cacheDir = join(appDataPath2, "launcher", "installers");
  const installerPath = join(cacheDir, `${loaderVersion}-installer.jar`);
  if (!existsSync(installerPath)) {
    const url = isNeoForge ? `https://maven.neoforged.net/releases/net/neoforged/neoforge/${ver}/neoforge-${ver}-installer.jar` : `https://files.minecraftforge.net/net/minecraftforge/forge/${ver}/forge-${ver}-installer.jar`;
...
function registerGameHandlers(mainWindow2, appDataPath2) {
  ...
  ipcMain.handle("game:launch", async (_, modpack) => {
    ...
    try {
      ...
      const javaPath = await getOrDownloadJava(modpack.mcVersion, appDataPath2, (text) => {
...
function registerAdminHandlers() {
  ...
  ipcMain.handle("admin:saveModpack", async (_, key, meta, isNew) => {
    const url = isNew ? `${API_BASE}/admin/modpacks` : `${API_BASE}/admin/modpacks/${encodeURIComponent(meta.name)}`;
    const res = await fetch(url, {
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    x: validSaved ? saved.x : void 0,
    y: validSaved ? saved.y : void 0,
    center: !validSaved,
    resizable: true,
    frame: false,
    show: false,
